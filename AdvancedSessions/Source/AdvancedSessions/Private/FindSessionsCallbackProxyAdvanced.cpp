...
UFindSessionsCallbackProxyAdvanced* UFindSessionsCallbackProxyAdvanced::FindSessionsAdvanced(UObject* WorldContextObject, class APlayerController* PlayerController, int MaxResults, bool bUseLAN, EBPServerPresenceSearchType ServerTypeToSearch, FString HostSessionAddr, int32 HostSessionPort, const TArray<FSessionsSearchSetting> &Filters, bool bEmptyServersOnly, bool bNonEmptyServersOnly, bool bSecureServersOnly, int MinSlotsAvailable)
{
	...
	Proxy->HostSessionAddr = HostSessionAddr;
	Proxy->HostSessionPort = HostSessionPort;
	return Proxy;
}
...
void UFindSessionsCallbackProxyAdvanced::Activate()
{
	...
	if (Helper.IsValid())
	{
		...
		if (Sessions.IsValid())
		{
			...
			tem.HardSet("HostSessionAddr", HostSessionAddr, EOnlineComparisonOpRedux::Equals);
			tem.HardSet("HostSessionPort", HostSessionPort, EOnlineComparisonOpRedux::Equals);
