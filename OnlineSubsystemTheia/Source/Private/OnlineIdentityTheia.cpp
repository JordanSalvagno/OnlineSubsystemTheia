...
bool FUserOnlineAccountTheia::GetAuthAttribute(const FString& AttrName, FString& OutAttrValue) const
{
...
bool FUserOnlineAccountTheia::GetUserAttribute(const FString& AttrName, FString& OutAttrValue) const
{
...
bool FUserOnlineAccountTheia::SetUserAttribute(const FString& AttrName, const FString& AttrValue)
{
	const FString* FoundAttr = UserAttributes.Find(AttrName);
...
inline FString GenerateRandomUserId(int32 LocalUserNum)
{
	...
	const bool bForceUniqueId = FParse::Param( FCommandLine::Get(), TEXT( "StableTheiaID" ) );
...
bool FOnlineIdentityTheia::Login(int32 LocalUserNum, const FOnlineAccountCredentials& AccountCredentials)
{
	FString ErrorStr;
	TSharedPtr<FUserOnlineAccountTheia> UserAccountPtr;
	...
	else
	{
		...
		if (UserId == NULL)
		{
			...
			UserAccountPtr = MakeShareable(new FUserOnlineAccountTheia(RandomUserId));
		...
		else
		{
			...
			TSharedRef<FUserOnlineAccountTheia>* TempPtr = UserAccounts.Find(*UniqueIdStr);
			check(TempPtr);
...
bool FOnlineIdentityTheia::Logout(int32 LocalUserNum)
{
...
bool FOnlineIdentityTheia::AutoLogin(int32 LocalUserNum)
{
	FString LoginStr;
	FString PasswordStr;
	FString TypeStr;
...
TSharedPtr<FUserOnlineAccount> FOnlineIdentityTheia::GetUserAccount(const FUniqueNetId& UserId) const
{
	TSharedPtr<FUserOnlineAccount> Result;

	FUniqueNetIdString StringUserId(UserId);
	const TSharedRef<FUserOnlineAccountTheia>* FoundUserAccount = UserAccounts.Find(StringUserId);
...
TArray<TSharedPtr<FUserOnlineAccount> > FOnlineIdentityTheia::GetAllUserAccounts() const
{
	TArray<TSharedPtr<FUserOnlineAccount> > Result;
	
	for (TMap<FUniqueNetIdString, TSharedRef<FUserOnlineAccountTheia>>::TConstIterator It(UserAccounts); It; ++It)
	{
		Result.Add(It.Value());
	}
...
TSharedPtr<const FUniqueNetId> FOnlineIdentityTheia::GetUniquePlayerId(int32 LocalUserNum) const
{
...
TSharedPtr<const FUniqueNetId> FOnlineIdentityTheia::CreateUniquePlayerId(uint8* Bytes, int32 Size)
{
...
TSharedPtr<const FUniqueNetId> FOnlineIdentityTheia::CreateUniquePlayerId(const FString& Str)
{
	return MakeShareable(new FUniqueNetIdString(Str));
}

ELoginStatus::Type FOnlineIdentityTheia::GetLoginStatus(int32 LocalUserNum) const
{
	TSharedPtr<const FUniqueNetId> UserId = GetUniquePlayerId(LocalUserNum);
	if (UserId.IsValid())
	{
...
ELoginStatus::Type FOnlineIdentityTheia::GetLoginStatus(const FUniqueNetId& UserId) const 
{
...
FString FOnlineIdentityTheia::GetPlayerNickname(int32 LocalUserNum) const
{
	...
	return TEXT("TheiaUser");
}

FString FOnlineIdentityTheia::GetPlayerNickname(const FUniqueNetId& UserId) const
{
	return UserId.ToString();
}

FString FOnlineIdentityTheia::GetAuthToken(int32 LocalUserNum) const
{
	TSharedPtr<const FUniqueNetId> UserId = GetUniquePlayerId(LocalUserNum);
	if (UserId.IsValid())
	{
...
void FOnlineIdentityTheia::RevokeAuthToken(const FUniqueNetId& UserId, const FOnRevokeAuthTokenCompleteDelegate& Delegate)
{
	UE_LOG(LogOnline, Display, TEXT("FOnlineIdentityTheia::RevokeAuthToken not implemented"));
	TSharedRef<const FUniqueNetId> UserIdRef(UserId.AsShared());
	TheiaSubsystem->ExecuteNextTick([UserIdRef, Delegate]()
	{
		Delegate.ExecuteIfBound(*UserIdRef, FOnlineError(FString(TEXT("RevokeAuthToken not implemented"))));
	});
}

FOnlineIdentityTheia::FOnlineIdentityTheia(FOnlineSubsystemTheia* InSubsystem)
...
FOnlineIdentityTheia::~FOnlineIdentityTheia()
{
}

void FOnlineIdentityTheia::GetUserPrivilege(const FUniqueNetId& UserId, EUserPrivileges::Type Privilege, const FOnGetUserPrivilegeCompleteDelegate& Delegate)
{
	Delegate.ExecuteIfBound(UserId, Privilege, (uint32)EPrivilegeResults::NoFailures);
}

FPlatformUserId FOnlineIdentityTheia::GetPlatformUserIdFromUniqueNetId(const FUniqueNetId& UniqueNetId) const
{
...
FString FOnlineIdentityTheia::GetAuthType() const
{
	return TEXT("");
}
